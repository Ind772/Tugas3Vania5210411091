import 'package:vania/vania.dart';
import 'package:tugas_vania_indra/app/models/orders.dart';

class OrdersController extends Controller {
  Future<Response> index() async {
    try {
      final orders = await Orders().query().get();
      return Response.json(orders);
    } catch (e) {
      return Response.json(400);
    }
  }

  Future<Response> create(Request request) async {
    try {
      final body = await request.body;

      if (body["order_num"] == null ||
          body["cust_id"] == null ||
          body["order_date"] == null) {
        return Response.json({"error": "Ada field yang kosong"});
      }

      final orderDate = DateTime.tryParse(body["order_date"]);
      if (orderDate == null) {
        return Response.json(
          {"error": "Format order_date salah"},
        );
      }

      final order = Orders()
        ..orderNum = body["order_num"]
        ..custId = body["cust_id"]
        ..orderDate = orderDate;

      await order.query().insert({
        "order_num": order.orderNum,
        "cust_id": order.custId,
        "order_date": order.orderDate!.toString()
      });

      return Response.json({"success": true, "data": order.toMap()});
    } catch (e) {
      print("Error in create: $e");
      return Response.json(
        {"error": "Failed to create order", "detail": e.toString()},
      );
    }
  }

  Future<Response> show(Request request, int id) async {
    try {
      final order = await Orders().query().where("order_num", "=", id).first();

      return Response.json({"success": true, "data": order});
    } catch (e) {
      return Response.json({
        "error": "Bad Request",
        "message": e.toString(),
      });
    }
  }

  Future<Response> update(Request request, int id) async {
    try {
      final body = await request.body;

      final orderDate = DateTime.tryParse(body["order_date"]);
      if (orderDate == null) {
        return Response.json(
          {"error": "Format order_date salah"},
        );
      }

      final order = Orders()
        ..orderNum = body["order_num"]
        ..custId = body["cust_id"]
        ..orderDate = orderDate;

      final affectedRows = await Orders()
          .query()
          .where("order_num", "=", order.orderNum)
          .update({
        "cust_id": order.custId,
        "order_date": order.orderDate!.toString()
      });

      if (affectedRows == 0) {
        return Response.json({
          "error": "Not Found",
          "message": "Tidak ada order yang ditemukan dengan order_num"
        });
      }

      return Response.json({"success": true, "data": order.toMap()});
    } catch (e) {
      print("Error di update: $e");
      return Response.json({"error": "Bad Request", "message": e.toString()});
    }
  }

  Future<Response> destroy(Request request, int id) async {
    try {
      final order = await Orders().query().where("order_num", "=", id).delete();

      if (order == 0) {
        return Response.json({
          "error": "Not Found",
          "message": "Order dengan $id tidak ditemukan"
        });
      }

      return Response.json({"message": "Order berhasil dihapus"});
    } catch (e) {
      return Response.json({
        "error": "Bad Request",
        "message": e.toString(),
      });
    }
  }
}

final OrdersController ordersController = OrdersController();
